...
```{r setup}
library(tidyverse)
library(readxl) #double check but probably remove this
library(deSolve) #lsoda
library(GGally) #check to see what's used here
library(tidyr)
library(dplyr)
library(kableExtra)
library(mmc) #check to see what's used here
library(SimDesign) #check to see what's used here
```

[[repeat basic data description]]
...
[[add in equations and model description]]
...
NPoolModel_modInput.fn <- function(t, y, parms, ...){
  ...
  dy <- ifelse(t < parms$root_time, 
    ...
    parms$decay %*% y[1:nrow(parms$decay)] #otherwise decay stuff
  return(list(c(dy)))
}
```
...
```{r fitFirstPool}
if(!file.exists(file.path('outputs', "FirstSecondPool.Rdata"))){
  ...
  save.image(file=file.path('outputs', "FirstSecondPool.Rdata"))
}else{
  load(file.path('outputs', "FirstSecondPool.Rdata"))
}
